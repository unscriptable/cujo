...
cujo._base.CssTextParser = cujo.CssTextParser = function (/* Object */ cb) {
    ...
    var
        ...
        trans = {
            ...
            '{': block,
            '}': unblock,
            '/*': d.partial(skip, /\*\//g, false),
            '<!--': cd,
            '-->': cd,
            '"': d.partial(skip, /[^\\]"/g, true),
            "'": d.partial(skip, /[^\\]'/g, true)
        },
        ...
        switch (sd.state) {
            ...
            case 'media': // This is the start of a declaration rule in a valid media block.
                ...
                if (c && !sd.stop && cb.onSelector) {
                    d.every(d.isArray(sels) ? sels : [sels], function (sel) {
